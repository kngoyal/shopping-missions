...
def copy_app_to_s3(*op_args) -> log:
    ...
    # Upload runner for data preprocessing to s3
    load_file_to_s3(
        file_name="{}{}".format(config["app"]["PathToRunners"], config["app"]["PreprocessRunner"]),
        bucket=config["s3"]["Bucket"],
        aws_credentials_id=config["airflow"]["AwsCredentials"],
        object_name="application/{}".format(config["app"]["PreprocessRunner"]),
    )

    # Upload runner for tuning model to s3
    load_file_to_s3(
        file_name="{}{}".format(config["app"]["PathToRunners"], config["app"]["TuneModelRunner"]),
        bucket=config["s3"]["Bucket"],
        aws_credentials_id=config["airflow"]["AwsCredentials"],
